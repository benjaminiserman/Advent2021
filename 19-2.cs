...
for (int i = 0; i < solved.Count; i++)
{
    for (int j = 0; j < solved.Count; j++)
    {
        if (i != j)
        {
            distances.Add(Math.Abs(solved[i].position.x - solved[j].position.x) + Math.Abs(solved[i].position.y - solved[j].position.y) + Math.Abs(solved[i].position.z - solved[j].position.z));
        }
    }
}
