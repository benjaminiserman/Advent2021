...
foreach (Cuboid instruction in instructions)
{
	...
	{
		for (int j = 0; j < newCuboid.Count; j++)
		{
			if (cuboids[i].on == newCuboid[j].on && newCuboid[j].Contains(cuboids[i]))
			{
				cuboids.RemoveAt(i--);
			}
			else if (Cuboid.Intersect(newCuboid[j], cuboids[i]))
			{
				Cuboid a = newCuboid[j];
				Cuboid b = cuboids[i];

				newCuboid.RemoveAt(j);
				cuboids.RemoveAt(i);
				j = 0;
				//i = 0; j = 0;
	...
	foreach (Cuboid c in newCuboid) if (c.on) cuboids.Add(c);
